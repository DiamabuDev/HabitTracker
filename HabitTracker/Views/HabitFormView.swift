...
struct HabitFormView: View {
    ...
    init(viewModel: HabitViewModel) {
        ...
        _viewModel = StateObject(wrappedValue: HabitFormViewModel(appViewModel: viewModel))
    }
    
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                MainTitle(title: "createNewHabit")
            ...
            ForEach(HabitType.allCases, id: \.self) { type in
                ...
                Button {
                    ...
                    Text(type.localized)
                        ...
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 12)
                        .background(isSelected ? .primaryPurple : .white)
                }
            }
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ForEach(viewModel.weekdayLabels, id: \.day) { item in
                    let isSelected = viewModel.selectedDays.contains(item.day)
                    Button {
                        ...
                        Text(item.labelKey)
                            ...
                            .background(isSelected ? .primaryPurple : .gray.opacity(0.3))
                            .clipShape(Circle())
                    }
                }
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 12) {
                ForEach([TimeOfDay.morning, .afternoon, .evening], id: \.self) { time in
                    let isSelected = viewModel.selectedTimeOfDay == time
                    Button {
                        ...
                        Text(time.localizationKey)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 20)
                                    .stroke(isSelected ? .primaryPurple : .gray.opacity(0.3), lineWidth: 2)
                            )
                            .cornerRadius(20)
                    }
                }
...
#Preview {
    HabitFormView(viewModel: HabitViewModel())
}
